...
class ProgressService: ObservableObject {
    ...
    private func generateStrengthData() {
        ...
        for workout in allWorkouts {
            for exercise in workout.exercises {
                if selectedExercise == "All Exercises" || exercise.exerciseName == selectedExercise {
    ...
    func getWeeklyStats() -> (workouts: Int, volume: Int, avgDuration: Int) {
        ...
        let avgDuration = weeklyWorkouts.isEmpty ? 0 : weeklyWorkouts.reduce(0) { $0 + ($1.duration ?? 0) } / weeklyWorkouts.count
        
        return (workouts: weeklyWorkouts.count, volume: totalVolume, avgDuration: avgDuration)
    }
