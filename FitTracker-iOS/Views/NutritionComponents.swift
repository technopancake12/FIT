...
struct MealSectionCard: View {
    ...
    private var mealNutrition: (calories: Double, protein: Double, carbs: Double, fat: Double) {
        ...
        for meal in meals {
            if let food = nutritionService.findFood(by: meal.foodId) {
...
struct MealEntryRow: View {
    ...
    private var food: Food? {
        nutritionService.findFood(by: meal.foodId)
    }
    ...
    private var calculatedNutrition: (calories: Double, protein: Double, carbs: Double, fat: Double)? {
        ...
        return nutritionService.calculateNutritionForAmount(food: food, amount: meal.amount)
    }
}
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 2), spacing: 8) {
                ...
                NutritionFactItem(label: "Protein", value: "\(food.protein, specifier: "%.1f")g")
                NutritionFactItem(label: "Carbs", value: "\(food.carbs, specifier: "%.1f")g")
                NutritionFactItem(label: "Fat", value: "\(food.fat, specifier: "%.1f")g")
            }
        }
        .padding()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 2), spacing: 8) {
                ...
                NutritionFactItem(
                    label: "Protein",
                    value: "\(food.protein * amount / 100, specifier: "%.1f")g"
                )
                NutritionFactItem(
                    label: "Carbs",
                    value: "\(food.carbs * amount / 100, specifier: "%.1f")g"
                )
                NutritionFactItem(
                    label: "Fat",
                    value: "\(food.fat * amount / 100, specifier: "%.1f")g"
                )
            }
        }
        .padding()
        ...
        NavigationView {
            VStack(spacing: 20) {
                ...
                Button("Manual Entry Instead") {
                    isPresented = false
                    // This would trigger showing the food search instead
                }
                .padding()
                .background(Color.blue)
                .foregroundColor(.white)
