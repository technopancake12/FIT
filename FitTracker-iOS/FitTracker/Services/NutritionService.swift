...
class NutritionService: ObservableObject {
    ...
    func addMealEntry(foodId: String, amount: Double, mealType: MealEntry.MealType, notes: String? = nil) {
        guard let food = foodDatabase.findFood(by: foodId) else { return }
        
        let foodEntry = FoodEntry(food: food, servingSize: amount)
        let meal = MealEntry(
    ...
    func updateMealEntry(mealId: String, amount: Double? = nil, notes: String? = nil) {
        if let index = meals.firstIndex(where: { $0.id == mealId }) {
            ...
            if let amount = amount, !updatedFoods.isEmpty {
                let firstFood = updatedFoods[0]
                updatedFoods[0] = FoodEntry(id: firstFood.id, food: firstFood.food, servingSize: amount)
            }
    ...
    func updateDailyNutrition(for date: Date = Date()) {
        ...
        for meal in dayMeals {
            ...
            for foodEntry in meal.foods {
                totalFiber += ((foodEntry.food.fiber ?? 0) * foodEntry.actualServingSize) / 100.0
            }
        }
    ...
    func calculateMacroPercentages() -> MacroPercentages {
        guard let nutrition = dailyNutrition, nutrition.totalCalories > 0 else {
            return MacroPercentages(protein: 0, carbs: 0, fat: 0)
        }
        
        let proteinCals = nutrition.totalProtein * 4
        let carbsCals = nutrition.totalCarbs * 4
        let fatCals = nutrition.totalFat * 9
        
        return MacroPercentages(
            protein: Int((proteinCals / nutrition.totalCalories) * 100),
            carbs: Int((carbsCals / nutrition.totalCalories) * 100),
            fat: Int((fatCals / nutrition.totalCalories) * 100)
        )
    }
